...
var svgCCL = d3.select("#comparison-cases-log-div").append("svg")
...
var maxNumDays = 0;
// Comparison Cases Log X
var cclX = 0;
// Comparison Cases Log Y
var cclY = 0;

d3.csv("data/countries_progression.csv", (data) => {

  establishData(data, "cases");
  ...
  for (let nation of nations) {
    ...
    if (nation["hidden-ccl"] == true) continue;
  ...
  function mousemove() {
    let x0 = cclX.invert(d3.mouse(this)[0]);
    ...
    updateCurrNum(i, data, "hidden-ccl", "totalNumCases");
    ...
    for (let nation of nations) {
      ...
      d3.select("#" + graph + "-" + focusId)
        .attr("cx", () => (data[i].day < maxDay) ? cclX(data[i].day) : (cclX(maxDay)) )
        .attr("cy", () => (data[i].day < maxDay) ? cclY(data[i][label]) : (cclY(totalNumCases)) );
  ...
  cclY = d3.scaleLog()
    .range([height, 1])
    .base(10)
    .domain([1, maxNumCases]);

  // Append the rates of duplication lines
  appendDuplicationLines(cclX, cclY, graph, svgCCL, maxNumCases);
  ...
  svgCCL.append("g")
    ...
    .call(d3.axisLeft(cclY)
      .tickFormat((d) => formatNum(d))
      .ticks(3)
      .tickSize(-width));

  // Append the nations' lines, focuses & labels
  appendLinesFocusesLabels(cclX, cclY, graph, svgCCL, "cases");

  // Append the legend
  appendLegend(graph, svgCCL);
