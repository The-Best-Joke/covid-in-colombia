...
var svgCOLD = d3.select("#colombia-deaths-div")
  .append("svg")
...
// Read the data
d3.csv("data/total_deaths_per_day.csv",
  ...
  (data) => {
    ...
    function mousemove() {
      ...
      focusDeaths
        ...
        .attr("y", y(selectedData.deaths) - 15)
        .html("Deaths: " + formatNum(selectedData.deaths));
      focusDate
        .attr("x", x(selectedData.date) - 25)
        .attr("y", y(selectedData.deaths) - 30)
        .html("Date: " + selectedData.date.getDate() + "/" + selectedMonth + "/" + selectedData.date.getFullYear());
        } 

    function mouseout() {
      focus.style("opacity", 0);
      focusDeaths.style("opacity", 0);
    ...
    var focusDeaths = svgCOLD
      .append("text")
        .attr("class", "focus-text");

    // Create the text that travels along the curve of chart
    var focusDate = svgCOLD
      .append("text")
        .attr("class", "focus-text");
